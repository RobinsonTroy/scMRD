...
Cells are then assigned to clusters based on the available SNP information matching perfectly to one cluster, while being a Hamming distance of >=3 to every other cluster. For some multiplexed runs, the maximum Hamming distance between clusters was <3, in which case we reduced the classification filter to accommodate this. In scMRD run MRD5, none of the clusters had a Hamming distance <3. 
```{r}
...
Subset the filtered NGT based on the cells in each cluster and store as individual samples.
```{r}
