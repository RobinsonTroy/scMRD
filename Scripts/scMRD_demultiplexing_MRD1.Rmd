---
title: "scMRD_demultiplexing_MRD1"
author: "TR"
date: "2023-05-01"
output: html_document
---

Run MRD1 is different from MRD2-6 in that it contains positive control spike-in cells from one AML sample. Since these cells are not useful for the manuscript analysis, we removed these cells before demultiplexing. For a more standardized approach to demultiplexing, refer to the code in "scMRD_demultiplexing. 
```{r, message=FALSE}
...
file <- c("/Users/robinst1/Desktop/MSKCC/Levine_Lab/Data/R/scDNA_analysis/WX_scDNA_h5_files/WX_MRD1.dna+protein.h5") #read in .h5 file
...
filtered_NGT_1 <- readRDS(file = "MRD1_filtered_NGT.rds")
...
#saveRDS(AF_matrix_subset, file = "MRD1_AF_matrix_subset.rds")

AF_matrix_subset<- readRDS(file = "MRD1_AF_matrix_subset.rds")

```

Extract and filter the AF matrix based on cutoffs described above. 
```{r, filtered AF matrix}
...
#saveRDS(filtered_AF, file = "MRD1_filtered_AF.rds")

filtered_AF_1 <- readRDS(file = "MRD1_filtered_AF.rds")

```


For this multiplexed run, we spiked in an AML sample (non-MRD, with known mutations) at a high percentage (>70%). These cells are not useful for our purposes here, so we will filter them out based on the presence of known mutations. After this step, there may still be a low number of residual cells present in the data, but we will be able to identify these cells and remove them at a later part of the analysis.  
```{r, }

filtered_NGT <- filtered_NGT_1[!(filtered_NGT_1$`chr21:36231792:C/T` == 1 | 
                                 filtered_NGT_1$`chr21:36231792:C/T` == 2 | 
                                 filtered_NGT_1$`chr2:209113113:G/A` == 1 | 
                                 filtered_NGT_1$`chr2:209113113:G/A` == 2 | 
                                 filtered_NGT_1$`chr2:25457242:C/T` == 1  | 
                                 filtered_NGT_1$`chr2:25457242:C/T` == 2 ),] %>%
                                 dplyr::filter(!is.na(.$Cell))

filtered_AF <- filtered_AF_1 %>% dplyr::filter(Cell %in% filtered_NGT$Cell)
...
intron_and_coding_SNPs <- rbind(coding_SNPs_final, intron_SNPs_final) %>% dplyr::filter(!CONSEQUENCE %in% c("nonsense","frameshift"))
...
n_required <- (nrow(filtered_NGT)*7)/1000

WT_HET_HOM_columns <- apply(candidate_SNPs, 2, function(x){
                            (sum(x < 20, na.rm  = TRUE) >= n_required & (sum(x >= 20  & x <= 80, na.rm  = TRUE) >= n_required |
                             sum(x > 80 & x <= 100, na.rm  = TRUE) >= n_required))}) %>% 
                             as.data.frame() %>% rename(. = "CONDITION_1") %>% 
                             rownames_to_column(., var = "id")


HET_HOM_columns <- apply(candidate_SNPs, 2, function(x){
...
SNPs_to_start<- final_mutation_info %>% subset(id %in% satisfied_conditions$id) %>%
                subset(!AA %in% "DNMT3A.V636L") %>% #this is a pathogenic variant that slipped through
                arrange(desc(GT_call_rate)) %>%
                head(20)
...
We then want to exclude extraneous SNPs that won't add value to our analysis. We achieve this by running a principal component analysis and calculating the proportion of variance contributed by each SNP. Applying a filter to select the SNPs that contribute a cumuluative proportion of variance >= 0.99 yields us 13 SNPs to use for demultiplexing. 
```{r}

PCA <- prcomp(complete_20_cells)
pov <- PCA$sdev^2 / sum(PCA$sdev^2)
cumulative_pov <- cumsum(pov)
top_PCS <- PCA$rotation[1:(which(cumulative_pov >= .99)[1]),] %>% as.data.frame()

nrow(top_PCS) #13 SNPs
```
...
Next, we run K-means clustering with the `clustering_AF_matrix`, setting the number of centers equal to the number of unique samples within the multiplex (in this case, 7 samples). For visualization purposes, we run UMAP analysis on SNP allele frequencies, with n_neighbors set to the square root of the total number of cells in the `clustering_AF_matrix`. 
```{r, kmeans + UMAP}

set.seed(1)

kmeans_AF <- kmeans(clustering_AF_matrix, centers = 7, nstart = 30)
...
umap_df$Cluster <- as.character(umap_df$Cluster)
           ...
           umap_df %>% ggplot(aes(x = UMAP1, y = UMAP2))+
                        geom_point(aes( fill = Cluster), color = "black",alpha = .6, size =2, shape =21)+
                        scale_fill_manual(values = rev(c(tol(7))))+
                        theme_classic()+
...
AF_cluster_list <- AF_cluster %>% split(AF_cluster$Cluster) #split dataframe into a list of cells in each cluster

sampled_pool_list <- lapply(AF_cluster_list,  function(df, n = min(cluster_ncell$n_cells)){ #this will be useful later on
                                          sampled_cells <- df %>% 
...
sampled_pool <- sampled_pool_list %>% do.call(rbind,.) #combines sampled cells into a single dataframe
dim(sampled_pool)
...
We'll generate enough doublets to make up roughly 5-10% of the total dataset. We start by simulating doublets at 10% of the total data, and then we remove homotypic doublets (i.e. doublets created by two cells from the same cluster).
```{r}
...
real_and_mixed_clusters$Cluster[real_and_mixed_clusters$Cluster == "1" | 
                                ...
                                real_and_mixed_clusters$Cluster == "5" |
                                real_and_mixed_clusters$Cluster == "6" |
                                real_and_mixed_clusters$Cluster == "7" ] <- "real"
...
cells_and_sampled_AF <- rbind(clustering_AF_matrix, sampled_all_heterotypic) #combine real and artificial cells with SNP AFs
...
umap_df_real_and_mixed$Cluster <- as.character(umap_df_real_and_mixed$Cluster)
                  ...
                  umap_df_real_and_mixed %>% ggplot(aes(x = UMAP1, y = UMAP2))+
                   geom_point(aes( fill = Cluster), color = "black",alpha = .4, size =2, shape =21)+
...
dist_C6_mix <- pdist(AF_cluster_list[6]$`6`[1:ncol(AF_cluster)-1], kmeans_sampled_center) %>% 
              as.matrix() %>% as.data.frame() %>% rename("V1"= "mix")

dist_C7_mix <- pdist(AF_cluster_list[7]$`7`[1:ncol(AF_cluster)-1], kmeans_sampled_center) %>% 
              as.matrix() %>% as.data.frame() %>% rename("V1"= "mix")
...
rownames(dist_C6_mix) <- rownames(AF_cluster_list[6]$`6`)
rownames(dist_C7_mix) <- rownames(AF_cluster_list[7]$`7`)
...
C6_mean_AF <- kmeans_AF$centers[6,]
C7_mean_AF <- kmeans_AF$centers[7,]
...
dist_C6_real <- pdist(AF_cluster_list[6]$`6`[1:ncol(AF_cluster)-1], C6_mean_AF) %>% 
                as.matrix() %>% as.data.frame() %>% rename("V1"= "real")

dist_C7_real <- pdist(AF_cluster_list[7]$`7`[1:ncol(AF_cluster)-1], C7_mean_AF) %>% 
                as.matrix() %>% as.data.frame() %>% rename("V1"= "real")
...
rownames(dist_C6_real) <- rownames(AF_cluster_list[6]$`6`)
rownames(dist_C7_real) <- rownames(AF_cluster_list[7]$`7`)
...
km_singlets <- kmeans(clusters_singlets, centers = 7, nstart = 30)
...
umap_singlets_gg<-  umap_df_singlets %>% ggplot(aes(x = UMAP1, y = UMAP2))+
                    geom_point(aes( fill = Cluster), color = "black",alpha = .5, size =2, shape =21)+
                    scale_fill_manual(values = rev(c(tol(7))))+
                    theme_classic()+
...
count(ordered_singlets$Cluster == 1)
count(ordered_singlets$Cluster == 2)
count(ordered_singlets$Cluster == 3)
count(ordered_singlets$Cluster == 4)
count(ordered_singlets$Cluster == 5)
count(ordered_singlets$Cluster == 6)
count(ordered_singlets$Cluster == 7)

```
Now we want to remove the residual AML cells. We can determine the SNP profile of the AML sample and see if it is present in our data. 
```{r}

AML_SNP_profile <- filtered_NGT_1[(filtered_NGT_1$`chr21:36231792:C/T` == 1 | 
                                   filtered_NGT_1$`chr2:209113113:G/A` == 1 | 
                                   filtered_NGT_1$`chr2:25457242:C/T` == 1 ),] %>%
                                   dplyr::select(c(Cell, colnames(clustering_AF_matrix))) %>% 
                                   dplyr::filter(!is.na(.$Cell)) %>%
                                   column_to_rownames(., var = "Cell") %>% 
                                   t() %>% as.data.frame() %>%
                                   apply(.,1,function(x) names(which.max(table(x)))) %>% as.data.frame() %>% rename("." = "AML_SNP_profile")
        
```

We can see that comparing the SNP profile to the heatmap, the AML sample is cluster 5. We will remove cluster 5 from the data. 
Now subset the data and re-run Kmeans clustering with the data frame consisting of singlets with the AML sample removed
```{r}

AML_removed_singlets <- NGT_cluster_singlets %>% subset(!Cluster == 5) %>% dplyr::select(!Cluster)

```

Since we have now completely removed the spike-in AML sample, we reduce k to k = 6 for reclustering. 
```{r}

set.seed(1)

km_singlets_noAML <- kmeans(AML_removed_singlets, centers = 6, nstart = 30)

umap_singlets_noAML <- umap(AML_removed_singlets, n_neighbors = sqrt(nrow(AML_removed_singlets))) 
cell_cluster_singlets_noAML <- km_singlets_noAML$cluster %>% as.data.frame() %>% rename(. = "Cluster")

umap_df_singlets_noAML <- umap_singlets_noAML$layout %>%
                          as.data.frame() %>% 
                          merge(., cell_cluster_singlets_noAML, by = 0) %>% 
                          column_to_rownames(., var = "Row.names")


names(umap_df_singlets_noAML)[names(umap_df_singlets_noAML)== "V1"]<-"UMAP1"
names(umap_df_singlets_noAML)[names(umap_df_singlets_noAML)== "V2"]<-"UMAP2"
umap_df_singlets_noAML$Cluster <- as.character(umap_df_singlets_noAML$Cluster)

umap_singlets_no_AML_gg<-  umap_df_singlets_noAML %>% ggplot(aes(x = UMAP1, y = UMAP2))+
                           geom_point(aes( fill = Cluster), color = "black",alpha = .9, size =2, shape =21)+
                           scale_fill_manual(values = rev(c(tol(7))))+
                           theme_classic()+
                           theme(plot.title = element_text(hjust = 0.5, size = 14))+
                           labs(x = "UMAP1",
...
umap_singlets_no_AML_gg

```

```{r}

singlet_cluster_noAML_df <- km_singlets_noAML$cluster %>% as.data.frame() %>% rename(. = "Cluster")

NGT_cluster_singlets_noAML <- filtered_NGT %>%
                              column_to_rownames(.,  var ="Cell") %>% 
                              merge(.,singlet_cluster_noAML_df, by = 0, all =FALSE ) %>%
                              column_to_rownames(., var = "Row.names") %>% 
                              .[c(colnames(AML_removed_singlets),"Cluster")]

superheat(NGT_cluster_singlets_noAML[, 1:ncol(NGT_cluster_singlets_noAML)-1],
          scale = FALSE,
          bottom.label.text.angle = 90,
          membership.rows = NGT_cluster_singlets_noAML$Cluster,
          bottom.label.size = 0,
          bottom.label.text.size = 2.5,
          left.label.text.size = 2.8,
          left.label.size = .1,
...
ordered_singlets_noAML<- NGT_cluster_singlets_noAML %>% arrange(Cluster)

count(ordered_singlets_noAML$Cluster == 1)
count(ordered_singlets_noAML$Cluster == 2)
count(ordered_singlets_noAML$Cluster == 3)
count(ordered_singlets_noAML$Cluster == 4)
count(ordered_singlets_noAML$Cluster == 5)
count(ordered_singlets_noAML$Cluster == 6)


    pheatmap(ordered_singlets_noAML[, 1:ncol(ordered_singlets_noAML)-1], 
             show_rownames = FALSE, 
             show_colnames = FALSE,
             cluster_rows = FALSE, 
             cluster_cols = TRUE, 
...
singlets_AF <- clustering_AF_matrix[rownames(singlet_cluster_noAML_df),] %>% 
              cbind(., singlet_cluster_noAML_df) %>% 
              split(.$Cluster) 
...
C6_singlets <- singlets_AF$`6`[1:ncol(clustering_AF_matrix)]
...
C6_to_match <- matching_df[,rownames(C6_singlets)] %>% apply(., 2, as.character) %>% as.data.frame()
  ...
  rownames(C6_to_match) <- colnames(C6_singlets)
...
C6_input <- apply(C6_to_match,1,function(x) names(which.max(table(x)))) %>% as.data.frame() %>% rename("." = "C6")
...
C6_matched <- match_cells(cluster = C6_input, x = matching_df_split_noNA, names = matching_NGT)

```

Now we'll calculate the Hamming distance between the clusters themselves. This will ensure that if there are clusters that have highly similar SNP profiles (i.e. Hamming distance <3) we can account for it in the downstream filtering steps. From the heatmap we can see that Cluster 1 and Cluster 2 are highly similar, with a Hamming distance of 1.
```{r}
...
C6_info <- C6_matched %>% rename("V1"= "C6")
...
match_master <- cbind(C1_info, C2_info) %>%
               ...
               cbind(.,available_info_singlets) %>%
                  ...
                  mutate(C6_abs = (possible_matches - C6)) 

```

Cells are then assigned to clusters based on the available SNP information matching perfectly to one cluster, while being a Hamming distance of ≥3 to every other cluster. For clusters which have a Hamming distance <3 to one another (i.e. Cluster 1 and Cluster 2), we reduce the classification filter to accomodate this. 
```{r}

C1_filtered<- match_master %>% 
              dplyr::filter(C1_abs >= possible_matches)  %>% #must match perfectly to cluster 1
              dplyr::filter((C1_abs - C2_abs) >= 1 & #must be Hamming distance of >=1 to cluster 2
                            (C1_abs - C3_abs) >= 3 & #must be Hamming distance of >=3 to cluster 3-6
                            (C1_abs - C4_abs) >= 3 &
                            (C1_abs - C5_abs) >= 3 &
                            (C1_abs - C6_abs) >= 3)
...
C6_filtered<- match_master %>% 
              dplyr::filter(C6_abs >= possible_matches)  %>%
              dplyr::filter((C6_abs - C1_abs) >= 3 & 
                            (C6_abs - C2_abs) >= 3 & 
                            (C6_abs - C3_abs) >= 3 &
                            (C6_abs - C4_abs) >= 3 &
                            (C6_abs - C5_abs) >= 3)
...
```{r, subset final cells per cluster}
...
C6_final<-subset(filtered_NGT, Cell %in% rownames(C6_filtered),)
...
Cluster_check_6_matched <- detect_mutations(C6_final)

Cluster_check_filter_6_matched <- Cluster_check_6_matched %>%
                                  dplyr::filter(GT_call_rate > 10) %>%
                                  dplyr::select(Bulk_VAF, id)

names(Cluster_check_filter_6_matched)[names(Cluster_check_filter_6_matched) == "Bulk_VAF"]<-"C6_VAF"
...
clusters_for_hm_matched<- merge(ref_mut,Cluster_check_filter_1_matched, by = "id", all = T ) %>%
                  ...
                  merge(., Cluster_check_filter_6_matched, by = "id", all = T) %>%
                  dplyr::filter(AA == "TP53.R273H" |   
                                AA == "NRAS.G12D" | 
                                AA == "DNMT3A.V636L"| 
                                AA == "NRAS.G12C"|
                                AA == "DNMT3A.R882C"| 
...
names(clusters_for_hm_matched)[names(clusters_for_hm_matched) == "C6_VAF"]<- "Cluster 6"
...
ncells_C1<- C1_filtered %>% nrow() %>% as.data.frame() %>% rename("." = "Cluster_1")
ncells_C2<- C2_filtered %>% nrow() %>% as.data.frame() %>% rename("." = "Cluster_2")
ncells_C3<- C3_filtered %>% nrow() %>% as.data.frame() %>% rename("." = "Cluster_3")
ncells_C4<- C4_filtered %>% nrow() %>% as.data.frame() %>% rename("." = "Cluster_4")
ncells_C5<- C5_filtered %>% nrow() %>% as.data.frame() %>% rename("." = "Cluster_5")
ncells_C6<- C6_filtered %>% nrow() %>% as.data.frame() %>% rename("." = "Cluster_6")
...
```
Calculate the proportion of cells successfully classified. The denominator here is the number of cells remaining after removing the majority of the spike-in AML sample. 
```{r}
...
Cluster 3 (NPM1-mut) is the only sample that is post-HSCT. The only non-mutated sample present is cluster 6, which we can infer to be the HSCT donor sample. 
```{r}

# sample_1 <- subset(filtered_NGT, Cell %in% C1_final$Cell)
# sample_2 <- subset(filtered_NGT, Cell %in% C2_final$Cell)
# sample_3 <- subset(filtered_NGT, Cell %in% c(C3_final$Cell, C6_final$Cell))
...
```{r}
  ...
  # write_rds(sample_1, file = "/Users/robinst1/Desktop/MSKCC/Levine_Lab/Data/R/scDNA_analysis/WX/MRD_Manuscript/WX_MRD1_v2_S1.rds")
  # write_rds(sample_2, file = "/Users/robinst1/Desktop/MSKCC/Levine_Lab/Data/R/scDNA_analysis/WX/MRD_Manuscript/WX_MRD1_v2_S2.rds")
  # write_rds(sample_3, file = "/Users/robinst1/Desktop/MSKCC/Levine_Lab/Data/R/scDNA_analysis/WX/MRD_Manuscript/WX_MRD1_v2_S3.rds")
  # write_rds(sample_4, file = "/Users/robinst1/Desktop/MSKCC/Levine_Lab/Data/R/scDNA_analysis/WX/MRD_Manuscript/WX_MRD1_v2_S4.rds")
  # write_rds(sample_5, file = "/Users/robinst1/Desktop/MSKCC/Levine_Lab/Data/R/scDNA_analysis/WX/MRD_Manuscript/WX_MRD1_v2_S5.rds")

```

For plotting purposes later, we'll combine the donor + host samples and recalulate mutation VAF. This is necessary if the goal is to compare the single cell VAF to bulk VAF. 
```{r}

C3_C6_final<-subset(filtered_NGT, Cell %in% c(rownames(C3_filtered), rownames(C6_filtered)),)

Cluster_check_C3_C6_matched <- detect_mutations(C3_C6_final)

Cluster_check_filter_C3_C6_matched <- Cluster_check_C3_C6_matched %>%
                                  dplyr::filter(GT_call_rate > 10) %>%
                                  dplyr::select(Bulk_VAF, id)

names(Cluster_check_filter_C3_C6_matched)[names(Cluster_check_filter_C3_C6_matched) == "Bulk_VAF"]<-"C3_C6_VAF"
...
clusters_for_hm_matched_recalculated<- merge(ref_mut,Cluster_check_filter_1_matched, by = "id", all = T ) %>%
                                        merge(., Cluster_check_filter_2_matched, by = "id", all = T) %>%
                                        merge(., Cluster_check_filter_4_matched, by = "id", all = T) %>%
                                        merge(., Cluster_check_filter_5_matched, by = "id", all = T) %>%
                                        merge(., Cluster_check_filter_C3_C6_matched, by = "id", all = T) %>%
                                        dplyr::filter(AA == "TP53.R273H" |   AA == "NRAS.G12D" | AA == "DNMT3A.V636L"| AA == "NRAS.G12C"|
                                                      AA == "DNMT3A.R882C" | AA == "RUNX1.R166G"| AA == "SRSF2.P95H"|
                                                      AA == "FLT3.D835Y"| AA == "RUNX1.R166Q"| 
                                                      AA == "IDH2.R140Q"| AA == "SRSF2.P95L"| AA == "JAK2.V617F"| AA == "SF3B1.K666T"|
                                                      AA == "RUNX1.R162G"| AA == "NPM1.287"| AA == "DNMT3A.592"| AA == "DNMT3A.R688C"|
                                                      AA == "NRAS.Q61H"| AA == "SETBP1.D868N")%>%
                                        column_to_rownames(., var = "AA") %>%
                                        dplyr::select(!c("id"))
                  

clusters_for_hm_matched_recalculated[is.na(clusters_for_hm_matched_recalculated)]<- 0
names(clusters_for_hm_matched_recalculated)[names(clusters_for_hm_matched_recalculated) == "C1_VAF"]<- "Cluster 1"
names(clusters_for_hm_matched_recalculated)[names(clusters_for_hm_matched_recalculated) == "C2_VAF"]<- "Cluster 2"
names(clusters_for_hm_matched_recalculated)[names(clusters_for_hm_matched_recalculated) == "C4_VAF"]<- "Cluster 4"
names(clusters_for_hm_matched_recalculated)[names(clusters_for_hm_matched_recalculated) == "C5_VAF"]<- "Cluster 5"
names(clusters_for_hm_matched_recalculated)[names(clusters_for_hm_matched_recalculated) == "C3_C6_VAF"]<- "Cluster 3_6"
...
clusters_for_decon <- clusters_for_hm_matched_recalculated %>% rownames_to_column(., var = "Mutations") %>%
                       reshape2::melt(.) %>% 
